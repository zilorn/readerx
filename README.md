...
- **设置** `/settings`：浅色 / 深色 / 护眼三套主题、正文字号、界面语言（跟随系统 / 简体中文 /
  English），分章规则在独立子页管理
- **同步** `/sync`（设置 → 同步）：同一局域网内的两台设备直接互相同步**书籍信息、阅读进度、
  书签、分组与书源**（正文与封面留在各自设备上，不在同步范围内）；可开启 / 关闭、用配对码
  连接设备、扫描局域网发现设备、手动同步或按间隔自动同步；已配对的设备可以从列表里删除
  （删除后本机不再与它同步、并拒绝它连进来，要恢复就在「查找局域网设备」里重新接受）；
  两台设备改到同一处内容时进冲突队列，逐条选择保留哪一份。不需要云服务，也不需要账号

...
- **设置** `/settings`：浅色 / 深色 / 护眼三套主题、正文字号、界面语言（跟随系统 / 简体中文 /
  English），分章规则在独立子页管理
- **同步** `/sync`（设置 → 同步）：同一局域网内的两台设备直接互相同步**书籍信息、正文、
  阅读进度、书签、章节目录、分组、书源、文本替换与分章规则**（封面不同步）：对端没有的
  章节按需搬运，本机还没导入的书会连元信息带正文一起落地成一本可读的书；可开启 / 关闭、
  用配对码连接设备、扫描局域网发现设备、手动同步或按间隔自动同步；已配对的设备可以从
  列表里删除（删除后本机不再与它同步、并拒绝它连进来，要恢复就在「查找局域网设备」里
  重新接受）；两台设备改到同一处内容时进冲突队列，逐条选择保留哪一份。
  不需要云服务，也不需要账号（对照表见 `docs/sync.md` 第 7.1 节）
- **阅读** `/book/:id`：章节阅读、上一章 / 下一章、目录抽屉、阅读进度跨页同步；桌面宽窗口下正文
  并排两页（手机与窄窗口仍是单页，列宽随可用宽度弹性调整），桌面端鼠标贴到阅读区上 / 下边缘
  呼出顶底菜单、贴右边缘呼出目录侧栏
...
前后端共用一套日志：Rust 侧由 `readerx-log` 统一写入
`<应用数据目录>/logs/<日期>/readerx-<时刻>.log`（一天一个目录、一次启动一个文件，
单文件 2 MB 轮转，各自保留最近 7 天 / 最多 32 MB），书源引擎、独立二进制与 WebView 里的
前端记录都汇到同一份文件。
**设置 → 调试 → 应用日志**可直接按级别筛选、切换文件、复制与清空，并能在「常规 / 详细」之间切换

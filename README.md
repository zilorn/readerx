...
地址与 body 中都可使用占位。POST 的 body 以 `{` 或 `[` 开头按 JSON 发送，否则按表单编码；服务端应直接返回音频字节（mp3/wav/ogg）。

**预热与缓存**：播放/预取时会把合成好的句子音频按「书籍 + 声源 + 句子」持久化（`tts-audio/<bookId>/`），同一本书同一声源下次直接读缓存、不再请求服务端；阅读页停在停止状态时也会预取当前章节后续若干句用于预热。可在**设置 → 听书缓存**查看占用并一键清除，或按书籍清除（删除书籍时自动清理）。

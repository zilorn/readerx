...
- **书源**：独立书源管理页（创建/编辑/启停/删除、JSON 导入导出、功能级开关）；书源以 JS 编写，在 Rust 内嵌 **Boa 引擎**沙箱运行，支持 async 规则；一次搜索同时运行多少个书源由用户全局设置控制（设置 → 书源 → 书源并发）；在线阅读按「当前章 ±5 章」窗口预取缓存、阅读视图只读取当前章 ±1 章，也支持一键批量下载全文离线阅读

...
- 返回 HTML：内含 `<img>` 时按出现顺序抽取图片并保留周围文字（纯文本无 `<img>` 时行为与旧版完全一致）。
  **段落文字中间的 `<img>` 留在该段的文字流里**（与文字同排、文字接着排下去），
  整段只有图或图片独立成块时按整行插图居中排版；
- 返回对象 `{ text?, images? }`（`images` 亦可写 `imgs`）：显式给出图片地址，`text` 作为图前正文，
  `images` 依次排在文字之后、各自独占一行（与旧版行为一致）。
...
- 「书源并发」是**用户级全局设置**（设置 → 书源），指一次搜索同时运行多少个书源；
  拉正文时它同时是**单源内部的并行章节数**（1–8，默认 3）：一章一个任务，由这么多 worker
  线程逐章领取。它不是书源 JSON 的字段。
  多个书源之间彼此独立：发现页搜索**有一个源返回就立刻显示该源的结果**，不会等其他源跑完，
  所以慢源 / 超时源只会拖慢自己的条目，书源不必为「统一返回」做额外处理。
  一轮搜索可以被用户中途作废（换词重搜 / 点「停止」）：发现页立刻不再向**尚未开始**的书源发请求，
  已经在飞的那几次调用会照常跑完（单次调用预算仍是 45s），结果被丢弃 —— 书源不必感知取消，
  也不应假设「被调用了就一定有人要这个结果」。
- 同一本书的正文拉取可以并存且互不阻塞：阅读时的**窗口预取**（当前章 ±5 章）、下载面板的
  **批量下载**（默认全书，也可只下第 x–y 章）、阅读设置的**重新加载本章**各持独立的停止与进度，
  **冲突时以用户操作为准** —— 读到哪一章就把那一章**插到队首**（不必排在几百章的队列后面），
  批量下载开始时窗口预取让位，正在重载的那一章其余任务跳过，正文回写「先到先得」（只有用户显式
  重载才覆盖已有正文），目录「覆盖更新」会让按旧下标在飞的正文作废。书源无需为并发做特殊处理，
  只是同一章可能因「重新加载 / 阅读焦点切换」被较快地重取一次。
- **正文逐章交付，不打包**：每章一个任务，取回一章立刻解析 / 落盘 / 计数 —— 不按 20 章一批，
  也不等整批回来，因此慢章或失败章不会拖住同一批里的其它章节；点「停止下载」后引擎立刻不再
  领取新章节（已取回的那部分照常保留），不会像整批那样丢掉一批已经下好的正文。

## 常见坑

- 顶层（函数体外）代码**每次「运行」都会执行**：引擎为每个 worker 线程建一次上下文并 `eval`
  全书源代码，同一次运行内的多章复用它（一次独立调用如搜索 / 目录 / 单章重取则新建上下文）。

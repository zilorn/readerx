...
书源可以归入分组，便于在书源多了之后筛选用哪些源。分组清单由应用自己保存，语义上不属于书源本体：

- 本机保存的书源文件里记 `groupId`（应用生成的分组 id）。**导出时不带 `groupId`**，改为带可读的
  `groupName`（如 `"groupName": "小说"`；未分组则省略该字段）。
- 导入时按 `groupName` 匹配本机分组：已有同名分组直接复用，没有则新建；确认页会列出将「新建 / 已有」
  的分组，并可整体关闭「按分组名导入分组」——关闭后新导书源落未分组、覆盖项保留本机分组。
- 删除分组时组内书源自动退回未分组（只改归属，不动书源内容）。
- 分组只影响书源管理页的筛选与「发现」页可用书源的范围（可按分组限定搜索 / 发现），
  不参与书源 JS 的执行，也不改变入口函数的调用方式。
...
在线书详情页的「重新拉取书籍信息」会把 `bookDetail` 返回的 `tags` 同步回书架：应用记着上次
书源写回的标签，只增删这部分，用户手编的标签不受影响；`bookDetail` 这次没返回 `tags`
（或返回空数组）时不动书架上的标签。
...
- 同一本书的正文拉取可以并存且互不阻塞：阅读时的**窗口预取**（当前章 ±5 章）、下载面板的
  **批量下载**、阅读设置的**重新加载本章**各持独立的取消与进度，**冲突时以用户操作为准** ——
  批量下载开始时窗口预取让位（其目标本就覆盖窗口范围），正在重载的那一章其余任务跳过，正文
  回写「先到先得」（只有用户显式重载才覆盖已有正文），目录「覆盖更新」会让按旧下标在飞的正文
  作废。书源无需为并发做特殊处理，只是同一章可能因「重新加载 / 阅读焦点切换」被较快地重取一次。

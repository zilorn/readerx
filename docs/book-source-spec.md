...
- 同一本书的正文拉取可以并存且互不阻塞：阅读时的**窗口预取**（当前章 ±5 章）、下载面板的
  **批量下载**（默认全书，也可只下第 x–y 章）、阅读设置的**重新加载本章**各持独立的取消与进度，
  **冲突时以用户操作为准** —— 批量下载开始时窗口预取让位（正在读的那一章不受所选范围限制，
  仍会被单独优先取回），正在重载的那一章其余任务跳过，正文回写「先到先得」（只有用户显式重载
  才覆盖已有正文），目录「覆盖更新」会让按旧下标在飞的正文作废。书源无需为并发做特殊处理，

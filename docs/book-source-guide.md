...
（见上一节）。给别人导入时对方只需：
书源管理 → 导入（选择文件 / 粘贴 / 输入 JSON 网址）→ 确认（含免责声明）。

导出的 JSON 会带可读的 `groupName`（不带本机分组 id）：对方导入时按名字匹配自己的分组，
没有同名分组就新建一个；导入确认页可以整体关掉「按分组名导入分组」，关掉后新导书源不归组、
覆盖已有书源时保留本机原有的分组。

把书源 JSON 传到网上（如 GitHub Gist / raw 链接）后，直接把该网址交给应用即可拉取导入，
无需下载文件；导入前同样会先列出冲突与免责声明供确认。

## 10. 导入入口

页头三个导入图标进入的都是同一条「确认导入」流程（冲突逐条可选 · 含免责声明）：

- **文件**（箭头入托盘）：选择本机的 `.json` / `.txt` 文件；
- **粘贴**（剪贴板）：打开抽屉即静默读一次剪贴板并预填，也可以直接在输入框里粘贴或手打 JSON。
  输入框里若是 http(s) 网址，按钮会变成「拉取并导入」，直接联网取回——手里是网址还是 JSON
  都不必先分辨；JSON 解析不了、没识别到书源、拉取失败都就地提示，不会带着空计划进确认页；
- **链接**：只输入网址、从网址拉取（手里只有网址时用它更直接）。

确认页里还有「继续粘贴导入」：想换一份内容重新解析时退回粘贴抽屉，不必先关掉再从头进来。
书源列表为空时的页面上也有「导入 JSON / 粘贴导入 / 从网址导入」按钮，入口同样可达。

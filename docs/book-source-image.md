...
图片在**用户读到这一章时**才逐张下载（先按图片地址请求，图片未就绪时正文显示占位）：
...
- 图片只是「还没下载」时显示转圈占位；**下载失败**的占位框带「重试」按钮，点一下立即重取
  （同一张图再次读到本章时也会自动再试一次，两次自动尝试之间有冷却，不会连环打图站）。
...
| 现象 | 排查 |
| --- | --- |
| 图片显示“图片缺失”占位框 | 该图下载失败（无网 / 超限 / 非图片响应）。可点占位框上的「重试」；仍失败时先确认 `chapterUrl` 可访问；图站防盗链的 Referer 域名与章节页不一致时，可在规则里先用 `http.get` 换到可直链的地址。 |

...
- 下载成功的图片随即以 data URL **写回本章本地缓存**：离线可读、本次运行内反复翻页不重复请求
  （与 EPUB 插图同一套存储），下次打开这本书也不再重新下载。
...
- **已缓存章节不受影响**：之前按纯文本缓存的章节照常阅读；旧版缓存下来的图片（data URL）继续可用。
- **数据格式向后兼容**：书源 `schemaVersion` 仍为 1；在线书的图片块新增可选字段 `remote`
  （图片原始网络地址，用于阅读时下载 / 重试），旧版客户端忽略它即可，
  新版读旧版书库也不会崩（旧数据按 `src` 直接渲染）。
- 图片以 data URL 存入本地书 JSON：体积随图片增长明显。**「下载全部正文」会把整本书的图都
  下载到本地（正文之后单独一趟）**，漫画/扫描书请预估磁盘占用后再用。

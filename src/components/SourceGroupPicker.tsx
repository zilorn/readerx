...
/** 底部抽屉：给一个书源选择（或新建）分组 */
export function SourceGroupPicker(props: SourceGroupPickerProps) {
  ...
  function createAndPick() {
    ...
    if (!name) return;
    const group = createSourceGroup(name);
    if (group) pick(group.id);
  }
...
function GroupRow(props: { label: string; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      <FolderIcon size={16} class="flex-none" />
      <span class="min-w-0 flex-1 truncate">{props.label}</span>
      {props.active && <CheckIcon size={18} class="flex-none text-accent" />}
    </button>
  );
}

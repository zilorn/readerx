 ...
 *   只有面板里选「关闭」或真的到点才解除；
 * - 预热：HTTP 引擎下由阅读页按需触发 —— 用户起播或打开听书面板后才预热当前章节
 *   窗口（只读书不听说时不发起合成请求）；也可 warmBook 批量把整本书逐句合成进
 *   按书籍的磁盘缓存（同书同声源下次直接命中，不再请求）。
...
export interface TtsPlayer {
   ...
   * 需由阅读页在用户进入听书后显式调用：不做「停着就预热」的自动触发，
   * 否则只读书不听说的用户也会被后台合成请求。
   */
  warmup: () => void;
  /**
...
export function createTtsPlayer(ctx: TtsPlayerCtx): TtsPlayer {
  ...
  /** 预热当前章节窗口（阅读页在用户进入听书后调用）：停止状态下把阅读位置往后的若干句合成为音频并写进缓存 */
